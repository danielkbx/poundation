...
class PCoordinate extends PObject implements \JsonSerializable
{
	...
	static public function createCoordinate($latitude, $longitude)
	{
		...
		if (is_float($latitude) && is_float($longitude)) {
			if (self::LATITUDE_MINIMUM <= $latitude && $latitude <= self::LATITUDE_MAXIMUM && self::LONGITUDE_MINIMUM <= $longitude && $longitude <= self::LONGITUDE_MAXIMUM) {
	...
	static public function createCoordinateFromString($value)
	{
		...
		if ($value->first() == '(' && $value->last() == ')') {
			...
			if ($components->count() == 2) {
				...
				$lon = $components->objectForIndex(1)->floatValue();
